...
- `--num_shards`: Total number of parallel workers for dataset sharding (default: 1)
- `--shard_idx`: Which shard this process handles, 0-indexed (default: 0)
  - Use these to split evaluation across multiple processes or machines
  - Example: `--num_shards 4 --shard_idx 0` processes every 4th puzzle starting from index 0
...
#### Model Configuration
- `--api`: API provider to use. Choices: `"openai"`, `"anthropic"`, `"anthropic_bedrock"`, `"deepseek"`, `"vllm"`, `"togetherai"`, `"openrouter"`, `"googleai"`, `"custom_openai"`. Default: `"openai"`.
- `--model`: Model name or path. Required.
- `--model_save_name`: Model name in the saved results. If not provided, uses `--model`.
- `--base_url`: Custom base URL for OpenAI-compatible APIs (required when `--api` is `"custom_openai"`)
- `--api_key`: API key for custom OpenAI-compatible APIs (optional, overrides environment variable)
- `--api_key_env`: Environment variable name containing the API key for custom APIs (default: `"CUSTOM_API_KEY"`)
...
- For Together AI: `TOGETHER_API_KEY`
- For OpenRouter: `OPENROUTER_API_KEY`
- For Google AI: `GOOGLE_API_KEY`
- For custom OpenAI-compatible APIs: set the variable named by `--api_key_env` (default: `CUSTOM_API_KEY`)
