...
def main():
    ...
    parser.add_argument("--dataset_root", type=str, default=None,
                        help="Path to local dataset directory. If provided, loads from local parquet files instead of Hugging Face.")
    ...
    parser.add_argument("--num_shards", type=int, default=1,
                        help="Total number of parallel workers for dataset sharding.")
    ...
    parser.add_argument("--api", type=str, default="openai",
                        choices=["openai", "anthropic", "anthropic_bedrock", "deepseek", "vllm", "togetherai", "openrouter", "googleai", "custom_openai"],
                        help="API to use.")
    ...
    parser.add_argument("--api_key", type=str, default=None,
                        help="API key for custom OpenAI-compatible APIs.")
    parser.add_argument("--api_key_env", type=str, default="CUSTOM_API_KEY",
                        help="Environment variable containing the API key for custom OpenAI-compatible APIs.")
    ...
    if args.shard_idx < 0 or args.shard_idx >= args.num_shards:
        raise ValueError(f"--shard_idx must be in range [0, {args.num_shards}), got {args.shard_idx}")

    # Validate custom OpenAI-compatible API arguments
    if args.api == "custom_openai" and args.base_url is None:
        raise ValueError("--base_url is required when --api is 'custom_openai'")
    ...
    if args.dataset_root:
        dataset_path = pathlib.Path(args.dataset_root) / args.dataset
        dataset = datasets.load_dataset(
            "parquet",
            data_files={"test": str(dataset_path / "*.parquet")},
            split="test",
        )
    else:
    ...
    if args.num_shards > 1:
        ilocs = list(ilocs)[args.shard_idx::args.num_shards]
    ...
    elif args.api == "custom_openai":
        api_key = args.api_key if args.api_key is not None else os.environ.get(args.api_key_env)
        client = openai.AsyncOpenAI(
            api_key=api_key,
