...
# Split one dataset across multiple workers
export API=openai
export MODEL=gpt-4o-mini-2024-07-18
export OPENAI_API_KEY=...
export DATA_SOURCE=challenge_100
export NUM_SHARDS=4
for SHARD_IDX in 0 1 2 3; do
    python -m eval.run \
        --dataset ${DATA_SOURCE} \
        --output_csv ../data/benchmark_results/${DATA_SOURCE}/${MODEL}_shard${SHARD_IDX}.csv \
        --api ${API} \
        --model ${MODEL} \
...
export API=custom_openai
export MODEL=your-model-id
export CUSTOM_API_KEY=...
python -m eval.run \
    --dataset challenge_100 \
    --output_csv ../data/benchmark_results/challenge_100/${MODEL}.csv \
    --api ${API} \
    --model ${MODEL} \
